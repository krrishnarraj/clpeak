...
| File | Purpose |
|------|---------|
| `cpu_peak.cpp` | `CpuPeak`: ctor, `applyOptions`, `runAll` (category-ordered dispatch), `runWorkload` (warmup + **MT clock-settle** + probe + `pickIters` timed batch via the pool; the settle runs the workload at the target thread count for min(budget/4, 500 ms) before timing because boost limits track core residency on moving averages -- without it a 3955WX read its 32-thread fp32 row 11% BELOW its own 16-thread number, which SMT cannot cause. ST skips it).  Validated on a 3955WX: every MT row rose 2-15% once the settle was proportional (fp32 MT +8.8%, AVX2 byte-scan MT +14.7%), and 1T/core and SMT MT converged to 2088 GFLOPS = 100.9% of the peak independently predicted from that machine's store-bound copy MT (4.04 GHz all-core x 16 cores x 32 flops/cy).  SMT ratio then came out 1.0002 -- zero gain, the correct answer for a chain that already saturates the FMA units; the earlier 1.044 was itself a settle artifact), `enumerate`, `printInventory` |
...
| `compute_common.h` | `emitCompute()` — runs a chain single-threaded (`ST`) and across all cores (`MT`), emits both metrics; `emitVariants()` — runs **every** ISA variant from a `kernelMenu()` slot as its own test (ISA appended to the display name, `isaSlug()` into the tag for unique result keys), or one untagged `Unsupported` test if none.  The reader-facing `ST`/`MT` notes (`EmitOptions::description`) are authored ONCE in `emitCompute()` — they mean the same thing in all ~25 tests that route through it, so never repeat them at a call site; the per-test `description` rides each `TestSpec` |
...
| `bandwidth.cpp` | `runDramBandwidth` (STREAM read/copy/triad), `runCacheBandwidth` (per-level L1/L2/L3, ST+MT, shared-cache MT sets split across threads; **plus the `L1 bandwidth (write / copy)` test** — same `Benchmark::CacheBandwidth` gate — exposing the store-port width and load:store split, e.g. Olympus/Vera's 4-load/2-store pipes; M1 Pro write ST ~95 GB/s ≈ 93% of the 2x16B store bound.  **The write set is a QUARTER of L1d, not half** — see the comment there: half is the small cores' entire L1 on a hybrid chip.  Also expect write MT to scale far worse than read MT on Apple (~2.5x vs ~7.7x): the store path has a shared ceiling that degrades with footprint, reproduced outside clpeak, not a measurement artifact). The read kernel is `kernels().readsum`, write/copy are `kernels().writefill`/`copybuf` (per-TU vector stores in `base_compute.h` — see the libc gotcha below); DRAM arrays sized off the **aggregate** L3 (`pickStreamFloats`) + parallel first-touch for NUMA-local placement. No `TransferBW`/memcpy test — on a CPU it just re-measures the STREAM copy path.  **The triad row IS vectorised — do not re-investigate that.**  Disassembly of the `$_3` lambda: arm64 gets 16 floats/iter, 4x unrolled, `ldp`/`fmla.4s`/`stp`; x86 gets 8 floats/iter, 2x unrolled `movups`/`mulps`/`addps` (SSE2 128-bit, no FMA, because `bandwidth.cpp` is in the core lib at the *baseline* ISA — which still clears DRAM by a wide margin, so it does not matter).  Clang also emits a runtime alias check with a scalar fallback; the arrays are distinct allocations so the fast path always wins.  Triad reads below copy on every platform (RFO traffic on the store to A is uncounted), but the row **collapses specifically on Windows** — triad/copy is 0.60-0.92 on Linux/macOS and 0.32-0.38 on both Windows runners, and on the *same* EPYC 7763 the 1-stream read and 2-stream copy are within ~10% Linux-vs-Windows while the 3-stream triad drops 46%.  Windows' TLB-miss row is 1.7x worse there too, so this looks like page-size/TLB pressure over three concurrent 256 MB streams (Linux backs them with THP, Windows does not), not a codegen problem |
...
| `microarch.cpp` | `runAtomics` (`Benchmark::Atomics`, `--atomics`): relaxed `fetch_add` in ns/op — `uncontended ST` (line in own L1) and `contended MT` (all cores on ONE line; system-wide wall/op).  `runBranchPenalty` (`Benchmark::BranchPenalty`, `--branch-penalty`, ST): sorted-vs-shuffled data-dependent branch; emits `predicted`/`random` ns/branch + derived `penalty` = delta ÷ 0.5 mispredicts/branch.  **The taken arm writes through `volatile`** so the branch can't be if-converted to csel/cmov (verified: hot loop is `tbnz` + conditional store; the store cost cancels between runs since both take the same elements).  `runStoreForward` (`Benchmark::StoreForward`, `--store-forward`, ST): dependent store->same-address-load->+1 chain, ns per roundtrip.  A cost probe, **not** a feature detector -- do not read ~1 cycle as proof of memory renaming.  Cross-platform reference, codegen verified on every one -- the row splits cleanly into three groups, which is what makes it worth keeping: **AMD/Apple ~1 cycle** (M1 Pro 0.31 ns = 1.00 cy, Zen 2 0.25 = 1.10, Zen 3 0.31 = 1.01), **Neoverse ~6 cycles** (N2 and Cobalt 100 both 1.77 ns), **Intel ~7.5 cycles** (Ice Lake Xeon 8370C 2.16 ns).  The Neoverse and Intel figures sit near the vendor-documented forwarding latencies, which is the corroboration that the AMD/Apple ~1 cycle is a real difference in the silicon rather than a measurement artefact -- but keep the caveat above: an address-rotation control separates M1 (4.05 cy when the chain alternates slots) from Zen 2 (1.29 cy), so those two reach the same number by different mechanisms.  **`volatile` alone does not keep the load and store apart**: GCC folds the body into one memory-destination `addq $0x1,(mem)` on x86-64 (verified by disassembly), which measures RMW throughput instead -- and read 1.1 cycles there, looking exactly like the thing it was supposed to detect.  The loaded value therefore passes through an empty `"+r"` asm constraint (`CPU_KEEP_IN_REG`) so it must land in a GPR between the two accesses; x86 then emits `movq`/`incq`/`movq` and ARM codegen is unchanged.  Re-check the disassembly on any new compiler.  `runSmtScaling` (`Benchmark::SmtScaling`, `--smt-scaling`, fp phase, gflops): the widest non-SSVE fp32 chain at 1 thread/physical core (dedicated `CpuThreadPool` pinned to primary siblings — sysfs core_id on Linux, GLPI RelationProcessorCore on Windows, group 0 only) vs all logical threads; ratio ~1.0 = statically partitioned SMT (Vera spatial MT), ~1.1-1.3 = conventional SMT.  Unsupported on no-SMT CPUs and macOS (no hard affinity).  A taken-branch-throughput test (2 taken/cycle on Olympus) was considered and DROPPED: it needs a JIT'd jump chain to measure honestly — portable C++ only measures loop overhead |
...
- **Bandwidth loops must not share an induction variable with their tail.**
  ...
  pass, and multi-versions the loop.
  (An earlier note here blamed `ldr q` -> `ldp q` fusion.  That was wrong:
  the CI codegen probe shows the fixed, full-speed kernel still emits 15
  `ldp q` and still reaches 47.6 B/cycle.  The 140-vs-153 measurement that
  suggested it was for `vld1q_f32_x2`, which lowers to `ld1 {v,v}` — a
  different instruction.  The win came from the loop shape alone.)  Computing `nblk = M / step` once and walking a single
