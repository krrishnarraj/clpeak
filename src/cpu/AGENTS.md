...
| `compute_common.h` | `emitCompute()` — runs a chain single-threaded (`ST`) and across all cores (`MT`), emits both metrics; `emitVariants()` — runs **every** ISA variant from a `kernelMenu()` slot as its own test (ISA appended to the display name, `isaSlug()` into the tag for unique result keys), or one untagged `Unsupported` test if none.  The `ST`/`MT` reading notes (`EmitOptions::description`) are authored ONCE in `emitCompute()` — never repeat them at a call site; the per-test `description` rides each `TestSpec` |
