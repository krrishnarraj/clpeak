...
| `microarch.cpp` | `runAtomics` (`Benchmark::Atomics`, `--atomics`): relaxed `fetch_add` in ns/op — `uncontended ST` (line in own L1) and `contended MT` (all cores on ONE line; system-wide wall/op).  `runBranchPenalty` (`Benchmark::BranchPenalty`, `--branch-penalty`, ST): sorted-vs-shuffled data-dependent branch; emits `predicted`/`random` ns/branch + derived `penalty` = delta ÷ 0.5 mispredicts/branch.  **The taken arm writes through `volatile`** so the branch can't be if-converted to csel/cmov (verified: hot loop is `tbnz` + conditional store; the store cost cancels between runs since both take the same elements).  `runStoreForward` (`Benchmark::StoreForward`, `--store-forward`, ST): dependent store->same-address-load->+1 chain through a volatile slot, ns/roundtrip — cores with hardware memory renaming collapse it to ~1 cycle (M1 Pro: 0.31 ns = 1.01 cyc; Olympus/Vera claims the same), classic forwarding is ~4-6 cycles.  **`volatile` alone does not keep the load and store apart**: GCC folds the body into one memory-destination `addq $0x1,(mem)` on x86-64 (verified by disassembly on a 3955WX), which measures RMW throughput, not a roundtrip -- and it read 1.1 cycles there, looking exactly like renaming when it was not.  The loaded value is therefore passed through an empty `"+r"` asm constraint (`CPU_KEEP_IN_REG`) so it must land in a GPR between the two accesses; x86 then emits `movq`/`incq`/`movq` and ARM codegen is unchanged.  Re-check the disassembly on any new compiler.  Post-fix reference with codegen verified on both: M1 Pro 0.31 ns (1.01 cyc), Zen 2 / 3955WX 0.25 ns (1.10 cyc) -- the Zen 2 result is well under that core's ~7-cycle documented forwarding latency, so it too appears to eliminate the roundtrip, but the address-rotation control has not been run there yet; treat 'renaming' as unconfirmed on Zen 2.  That 1 cycle on Apple is corroborated, not assumed: the same M1 Pro core measures **4.05 cyc** per roundtrip when the chain rotates between two slots instead of hammering one address, which is the un-renamed forwarding latency — renaming only fires when the load's address provably matches the pending store's.  If a future core reports ~1 cyc here, cross-check with the 2-slot variant before calling it renaming.  `runSmtScaling` (`Benchmark::SmtScaling`, `--smt-scaling`, fp phase, gflops): the widest non-SSVE fp32 chain at 1 thread/physical core (dedicated `CpuThreadPool` pinned to primary siblings — sysfs core_id on Linux, GLPI RelationProcessorCore on Windows, group 0 only) vs all logical threads; ratio ~1.0 = statically partitioned SMT (Vera spatial MT), ~1.1-1.3 = conventional SMT.  Unsupported on no-SMT CPUs and macOS (no hard affinity).  A taken-branch-throughput test (2 taken/cycle on Olympus) was considered and DROPPED: it needs a JIT'd jump chain to measure honestly — portable C++ only measures loop overhead |
