...
// advertised 615).  INT8 (wmma_int8_sparse.cu) and FP8 (wmma_fp8_sparse.cu)
// get their full 2x on this part too.  Every sparse kernel here reaches 2x by
// using the doubled-K shape against its dense counterpart -- see the shape note
// in wmma_int8_sparse.cu for what a same-K pairing measures instead.
//
