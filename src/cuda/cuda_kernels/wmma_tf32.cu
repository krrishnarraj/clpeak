// WMMA tf32xtf32+fp32 m16n16k8 -- Ampere+ tensor-core TF32 throughput via
// the nvcuda::wmma fragment API.  Mirrors wmma_fp16.cu's 4-chain structure;
// only the fragment dtype (precision::tf32) and K dimension (8) change.  Like
// wmma_fp16 this fragment path under-saturates consumer Blackwell (~10.7
// TFLOPS, flat vs chain count, ~half of cuBLASLt's ~20.6); the native
// mma.sync path (wmma_tf32_mma, m16n8k8) chases peak.
//
