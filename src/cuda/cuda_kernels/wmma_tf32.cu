// TF32 tensor-core throughput via inline mma.sync PTX at the native m16n8k8
// tile with fp32 accumulator.  Ampere+.
//
// This replaces the older nvcuda::wmma fragment (m16n16k8) path, which
// under-saturated consumer Blackwell badly (~10.7 TFLOPS, half of cuBLASLt's
// ~20.6).  The native mma.sync tile reaches peak (~21.3 on RTX 5060, at/above
// cuBLASLt); TF32 always accumulates in fp32, so there is no lower-precision
// "full-rate" accumulator to chase -- this IS the peak path.  Same layout and
// 8-chain non-volatile asm idiom as wmma_fp8_e4m3.cu.
//
// TF32 operands are passed as .b32 holding the fp32 bit pattern; the mma
// instruction consumes only the top 19 bits (tf32), so the raw float works.
//
