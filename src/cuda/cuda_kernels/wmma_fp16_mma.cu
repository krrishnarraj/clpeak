// FP16 tensor-core throughput via inline mma.sync PTX at the native
// m16n8k16 tile with fp32 accumulator.  Available sm_80+ (Ampere).
//
// Companion to the portable wmma_fp16 (nvcuda::wmma fragment, m16n16k16),
// which under-saturates consumer Blackwell (~42 TFLOPS, flat vs chain
// count -- roughly half of cuBLASLt fp16 ~78).  This is the same lesson as
// INT8: the fragment path leaves throughput on the table and the native
// mma.sync tile is the one that chases peak.  Same layout/idiom as
// wmma_fp8_e4m3.cu (m16n8k16 f16 packs identically to m16n8k32 f8).
//
// Per-thread fragment layout (32 threads/warp, A=row-major, B=col-major):
//   A: m16 x k16 = 512 bytes / 32 threads = 16 bytes/thread = 4 x .b32 (2 halfs)
//   B: k16 x  n8 = 256 bytes / 32 threads =  8 bytes/thread = 2 x .b32
//   C/D: m16 x n8 = 128 fp32 / 32 threads = 4 fp32/thread per accumulator
//
// EIGHT independent accumulator chains in a SINGLE non-volatile inline-asm
// block so ptxas can interleave the per-chain ops to hide MMA latency.
//
...
extern "C" __global__ void wmma_fp16_mma(float *out, float A)
{
