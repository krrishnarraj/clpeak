// BF16 tensor-core throughput via inline mma.sync PTX at the native
// m16n8k16 tile with fp32 accumulator.  Available sm_80+ (Ampere).
// Companion to the portable wmma_bf16 (nvcuda::wmma fragment) which
// under-saturates consumer Blackwell; identical layout to wmma_fp16_mma,
// only the operand type in the instruction (bf16) and the input packing
// change.
//
// Per-thread fragment layout (32 threads/warp, A=row-major, B=col-major):
//   A: m16 x k16 = 512 bytes / 32 threads = 16 bytes/thread = 4 x .b32 (2 bf16)
...
extern "C" __global__ void wmma_bf16_mma(float *out, float A)
{
    unsigned short h = __bfloat16_as_ushort(__float2bfloat16(A));
