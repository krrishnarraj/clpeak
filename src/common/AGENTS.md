...
Diagnostics ride the same stream. `RunLog` (`run_log.cpp`) is the host-owned
sink for one whole run: every `CLPEAK_LOG` / `CLPEAK_VLOG` line, every
...
- Emitting a diagnostic? → `CLPEAK_LOG(level, …)` / `CLPEAK_VLOG(…)` in `include/common/common.h`; the levels and where each ends up are documented there, the run-level store in `include/common/run_log.h`
...
| `logger.cpp` | Base `logger` class: result-scope API (`emit()`/`skip()`/`skipAll()`) dispatching `LogEvent`s to the single `onEvent()` hook; `log()` / `note()` stamp a diagnostic with the open scope, hand it to the `RunLog`, and dispatch it as a `Log` event; `TestEnd` stamps the test's `duration_s`. Also whitespace-collapses the documentation strings to one line |
| `logger_text.cpp` | `LoggerText` — renders the event stream as indented/aligned text with per-row units + baseline deltas, and under `--describe` the wrapped test/reading documentation (desktop CLI). Its own warnings/errors print inline; debug and library-sourced lines go to stderr under `--verbose` only, through `clpeak::stderrWrite()` so a console capture never re-captures them |
...
| `console_mute.cpp` | `clpeak::ScopedConsoleMute` — silences stdout+stderr at the fd level for a scope, so vendor runtimes that print below any log level (hipBLASLt's Tensile internals, the ONNX schema registry) cannot wreck the results table. Under `--verbose` it captures instead: a pipe drained by a thread, each line recorded as a `source: "console"` debug entry (first 200 per scope) and echoed to the real stderr. `stderrOnly` mutes only stderr (leaving a table on stdout visible) for a scope wide enough to catch a library's async worker-thread logging; `watch`/`sawWatched()` report whether the muted stream contained given phrases (e.g. a LiteRT accelerator announcing it was lost) even when the text is discarded |
...
| `keep_awake.cpp` | `clpeak::KeepAwake` — holds the machine out of idle sleep for one run; the CLI's and the FFI's run loops each own one. A Windows power request, a macOS IOKit assertion, and on Linux logind's and the desktop's inhibitors through a libsystemd loaded at run time; a no-op on Android/iOS, where the app holds the screen. What each platform takes and why: `include/common/keep_awake.h` |
...
- If you change `LogEntry` → it is serialized in one place (`logEntryToJson`) for the document, the sidecar and the loader, but the FFI event (`logger_ffi.cpp`) and the Dart `LogEntry` (`app/lib/src/model/run_document.dart`) mirror it by hand.
